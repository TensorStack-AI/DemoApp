            ...
            try
            {
                ...
                using (_cancellationTokenSource = new CancellationTokenSource())
                {
                    ...
                    if (model.Type == TextModelType.Phi3)
                    {
                        ...
                        var phiPpipeline = Phi3Pipeline.Create(providerCPU, model.Path, phiType);
                        _greedyPipeline = phiPpipeline;
                        _beamSearchPipeline = phiPpipeline;
                    }
                    else if (model.Type == TextModelType.Summary)
                    {
                        var summaryPipeline = SummaryPipeline.Create(provider, providerCPU, model.Path);
                        _greedyPipeline = summaryPipeline;
                        _beamSearchPipeline = summaryPipeline;
                    }
